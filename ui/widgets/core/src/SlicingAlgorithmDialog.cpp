#include "MantidUiWidgetsCore/SlicingAlgorithmDialog.h"
...
void SlicingAlgorithmDialog::initLayout() {
  ...
  /*
   Do not need to connect up Accept/Reject. This gets done automatically, and
...
/// Destructor
SlicingAlgorithmDialog::~SlicingAlgorithmDialog() { saveSettings(); }

/**
 Create a formatted string for the dimension input based on an existing
 dimension.
...
/**
 ...
 It is expected that inherited classes will be able to implement this method,
 but
 this class can not obviously define how the basis vectors could be
 ...
 * @param criticalChange : Indicates that the inputs are different in some
 * critical fashion
 * @param bForceForget : Force the use of inputworkspace dimensions when
...
/**
Event handler for changes so that recursion depth for the ouput workspace is
either taken
...
/*
Perform tasks that are almost identical for derived types except for visibility
switch.
